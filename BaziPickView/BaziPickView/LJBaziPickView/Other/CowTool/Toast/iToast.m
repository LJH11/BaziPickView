...
- (void) show:(iToastType) type
{
	...
	UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake( 5, 0, textSize.width + 5, textSize.height + 5)];
	...
	if (image) 
    {
		v.frame = CGRectMake(0, 0, image.size.width + textSize.width + 25, MAX(textSize.height, image.size.height) + 10);
		label.center = CGPointMake(image.size.width + 10 + (v.frame.size.width - image.size.width - 10) / 2, v.frame.size.height / 2);
	}
    else 
    {
		v.frame = CGRectMake(0, 0, textSize.width + 20, textSize.height + 10);
